...
struct Cli {
    ...
    #[arg(long, default_value = "data/sim3d_life/")]
    dir: String,
    /// Output directory for CSVs
    #[arg(long, default_value = "results/life/")]
    out: String,
}
...
fn main() {
    ...
    // ── Population CSV ────────────────────────────────────────────────────────
    let pattern = format!("{}/state_mcs*.json", cli.dir.trim_end_matches('/'));
    ...
    for path in &files {
        ...
        let mean_target = if living.is_empty() { 0.0 } else {
            living.iter().map(|c| c.target_volume as f64).sum::<f64>() / living.len() as f64
        };
